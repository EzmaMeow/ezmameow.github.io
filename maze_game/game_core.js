...
export class Level extends THREE.Scene {
    ...
    //should provide a getter for public use that return the object or null
    //these are to help remove them from memory and reduce file size
    //also they are more for static object or reuseable ones
    #materials = {};
    #geometries = {
    ...
    get_material(key, fallback = null, cache = true) {
        if (this.#materials[key]) { return this.#materials[key]; }
        if (cache && fallback) { this.#materials[key] = fallback; }
        return fallback;
    }
    get_geometry(key, fallback = null, cache = true) {
        if (this.#geometries[key]) { return this.#geometries[key]; }
        if (cache && fallback) { this.#geometries[key] = fallback; }
        return fallback;
    }
    get_texture(key, fallback = null, cache = true) {
        if (this.#textures[key]) { return this.#textures[key]; }
        if (cache && fallback) { this.#textures[key] = fallback; }
        return fallback;
    }
    ...
    get default_xborder_geo() {
        if (!(this.#geometries.xborder)) {
            this.#geometries.xborder = new THREE.BoxGeometry((this.level_image.image.height) * this.#cell_size.x, this.#cell_size.y, this.#cell_size.z);
        }
        return this.#geometries.xborder
    }
    get default_zborder_geo() {
        if (!(this.#geometries.zborder)) {
            this.#geometries.zborder = new THREE.BoxGeometry(this.#cell_size.x, this.#cell_size.y, (this.level_image.image.width) * this.#cell_size.z);
        }
        return this.#geometries.zborder
    }
    get default_area_geo() {
        if (!(this.#geometries.area)) {
            this.#geometries.area = new THREE.PlaneGeometry(this.level_image.image.height * this.#cell_size.x, this.level_image.image.width * this.#cell_size.z);
        }
        return this.#geometries.area
    }
    ...
    build_maze() {
        ...
        if (this.#geometries.maze) {
            this.#geometries.maze.dispose();
        }
        this.#geometries.maze = BufferGeometryUtils.mergeGeometries(geometries, true);
        if (this.maze_mesh) {
            this.maze_mesh.geometry = this.#geometries.maze;
            ...
            this.maze_mesh = new THREE.Mesh(this.#geometries.maze, this.default_wall_mat);
    ...
    #create_static_border(id) {
        ...
        if (id == this.static_object_ids[0]) {
            ...
            for (let i = 0; i < this.level_image.image.height; i++) {
                ...
                const plane = new THREE.PlaneGeometry(this.#cell_size.z, this.#cell_size.y); // +Z
                plane.translate(i * this.#cell_size.x,0, - this.#cell_size.z/2);
                geometries.push(plane);
            }
            const border_geo = BufferGeometryUtils.mergeGeometries(geometries, true);
            border = new THREE.Mesh(border_geo, this.default_wall_mat);
        }
        else if (id == this.static_object_ids[1]) {
            const geometries = [];
            for (let i = 0; i < this.level_image.image.height; i++) {
                const plane = new THREE.PlaneGeometry(this.#cell_size.z, this.#cell_size.y).rotateY(Math.PI); // -Z
                plane.translate(i * this.#cell_size.x,0, this.level_image.image.width*2 -this.#cell_size.z/2);
                geometries.push(plane);
            }
            const border_geo = BufferGeometryUtils.mergeGeometries(geometries, true);
            border = new THREE.Mesh(border_geo, this.default_wall_mat);
            ...
            const geometries = [];
            for (let i = 0; i < this.level_image.image.height; i++) {
                const plane = new THREE.PlaneGeometry(this.#cell_size.x, this.#cell_size.y).rotateY(-Math.PI / 2); // -Z
                plane.translate(this.level_image.image.height*2 -this.#cell_size.x/2, 0, i * this.#cell_size.z);
                geometries.push(plane);
            }
            const border_geo = BufferGeometryUtils.mergeGeometries(geometries, true);
            border = new THREE.Mesh(border_geo, this.default_wall_mat);
            ...
            for (let i = 0; i < this.level_image.image.height; i++) {
                const plane = new THREE.PlaneGeometry(this.#cell_size.x, this.#cell_size.y).rotateY(Math.PI / 2); // -Z
                plane.translate(-this.#cell_size.x/2, 0, i * this.#cell_size.z);
                geometries.push(plane);
            }
            const border_geo = BufferGeometryUtils.mergeGeometries(geometries, true);
            border = new THREE.Mesh(border_geo, this.default_wall_mat);
    ...
    get north_border() { return this.#create_static_border(this.static_object_ids[0]); }
    get south_border() { return this.#create_static_border(this.static_object_ids[1]); }
    get east_border() { return this.#create_static_border(this.static_object_ids[2]); }
    get west_border() { return this.#create_static_border(this.static_object_ids[3]); }
    ...
    dispose() {
        ...
        for (const [key, value] of this.#materials) {
            if (value) {
                value.dispose();
    ...
    load_resources() {
        ...
        if (!(this.#textures['texture'])) {
            this.#textures['texture'] = Level.loader.load('texture.png', (texture) => {
